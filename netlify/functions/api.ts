...
async function writeGitHubFile(path: string, content: string, sha: string | null): Promise<boolean> {
  const url = `https://api.github.com/repos/${REPO_OWNER}/${REPO_NAME}/contents/${path}`
...
async function saveChoice(choice: Choice): Promise<boolean> {
  const result = await fetchGitHubFile(CHOICES_PATH)
...
async function getHangzhouRecords(): Promise<HangzhouRecord[]> {
...
async function saveHangzhouRecord(record: HangzhouRecord): Promise<boolean> {
  const result = await fetchGitHubFile(HANGZHOU_PATH)
